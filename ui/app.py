...
# The engine polls continuously but only sends orders while STATE["running"]
# is True, so Start/Stop actually gates whether trades go out.
engine = (
    TradingEngine(is_running=lambda: STATE["running"])
...
def start_background_services():
    """
    Start the live WebSocket stream and the trading engine once. Safe to
    call multiple times. Called on startup so the UI shows live data as
    soon as it loads.
    """
    ...
    if engine is not None and not _engine_started.is_set():
        engine.start()
        _engine_started.set()
...
def api_engine_events():
    """Return the most recent signals/orders generated by the engine."""
    if engine is None:
        return jsonify({"connected": False, "events": []})

    return jsonify(
        {"connected": True, "events": list(engine.events)[-30:][::-1]}
    )


@app.route("/api/engine/trades")
def api_engine_trades():
    """Return closed round-trip trades (for P&L / hit-rate)."""
    if engine is None:
        return jsonify({"connected": False, "trades": []})

    return jsonify(
        {"connected": True, "trades": list(engine.trades)[-20:][::-1]}
    )


@app.route("/api/engine/metrics")
def api_engine_metrics():
    """Return cumulative P&L, drawdown, trade count and hit rate."""
    if engine is None:
        return jsonify({"connected": False})

    return jsonify({"connected": True, **engine.metrics()})
